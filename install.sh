...
ALL_BUNDLES=(agency feature bugfix fix idea plan implement architecture review qa release init-project do)
...
if (( DO_BUNDLES && ! DRY_RUN )); then
  ...
  hermes bundles list 2>/dev/null | grep -E 'agency|feature|bugfix|fix|idea|plan|implement|architecture|review|qa|release|init-project|do' || true
fi
