...
- [x] 1.2 Rewrite `fixtures/skill-registry/roots/defective/fixture-block-scalar/SKILL.md` to the real defect shape: `description: |` followed immediately by the next frontmatter key (no body). Verifies: the suite's defective case asserts `BLOCK-SCALAR` against it
- [x] 1.3 Add `fixtures/skill-registry/roots/valid/fixture-folded-legal/SKILL.md` — the exact shape that was wrongly flagged (a `>-` with a real body, like `context-architecture`). Verifies: the suite's valid case asserts `FLAG: ok` and the first body line as description
