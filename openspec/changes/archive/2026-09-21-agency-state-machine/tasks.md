...
- [x] 1.5 Implement the declared precision limits: an unavailable input (no run trace, `gh` missing, no remote configured) still yields the provable state plus a section naming every transition whose verdict could not be determined, with the reason. Verifies: `fixtures/agency-next/check.sh` prints `OK` for its `gh present but failing` case (a fake `gh` first on PATH, with node/openspec/jq still reachable), asserting the `PR_STATE_UNDETERMINED` state and exit 0
...
- [x] 1.7 Print, as part of the output, that the state is informational and that the gates remain the reviewer's and QA's verdicts. Verifies: `bin/agency-next --change <name> | grep -qi 'informational'` exits 0 (the footer is in the OUTPUT, not only in --help), and `fixtures/agency-next/check.sh` asserts it
- [x] 1.8 Add `fixtures/agency-next/check.sh`: builds throwaway repos (planning-complete/partial tasks, all tasks done with no review evidence, a stale review snapshot, no active change, `gh` absent) and asserts the public state, the precise state and the exit contract for each. Verifies: `fixtures/agency-next/check.sh` exits 0 with `failures=0` and at least one case run; an unrunnable suite (openspec CLI not on PATH) exits 3 with an explicit not-a-pass line — reported as a failure, never as green
