...
on. A mismatch MUST be reported as a blocker of the same class as the fingerprint mismatch
`rules/quality.md` already defines — the review describes content that is not what is being
delivered — and MUST NOT be silently accepted.
...
- **WHEN** the working tree's fingerprint or diff hash at delivery differs from the frozen
  snapshot's
- **THEN** the mismatch is reported as a blocker naming both values, and the work is re-reviewed
  rather than delivered on the stale evidence
