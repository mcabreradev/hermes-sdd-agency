# Agency skill mirror — completeness

## Purpose

The public repo mirrors the agency's process skills so a reader who follows
`sdd-agency-export` to fork a working agency gets the complete set — including the
skills that describe what "complete" means (`sdd-agency-export` itself,
`hermes-sdd-packaging`). The mirrored skills must stay byte-identical to the live
`~/.hermes` copies and public-ready (no personal paths, no credentials).
