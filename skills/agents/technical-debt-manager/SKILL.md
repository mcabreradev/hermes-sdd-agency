...
description: "Expert technical debt analyst for code health"
---

<!-- Claude tools declared upstream: `Read, Grep, Bash, TodoWrite, WebFetch`. Hermes equivalents: read → read_file / search_files; edit → patch; write → write_file; bash → terminal; grep / glob → search_files; webfetch / websearch → web_extract / web_search; task → delegate_task. -->
...
Converted from `~/.claude/agents/technical-debt-manager.md` by `hermes-add-agent`. Edit that file and re-run to sync; this copy
